"""Validate or upload the already-built BattleMaps release to CurseForge."""
import argparse
import json
import os
import re
...
def api(path, token, data=None, content_type='application/json'):
    request = urllib.request.Request(API + path, data=data, headers={
        ...
        'User-Agent': 'BattleMaps-GitHub-Actions',
    })
    try:
...
def main():
    ...
    if args.mode == 'preflight':
        toc = Path('BattleMaps.toc').read_text(encoding='utf-8-sig')
        interface_match = re.search(r'^## Interface:\s*(\d+)', toc, re.M)
        if not interface_match:
            sys.exit('BattleMaps.toc has no Interface field.')
        ...
        metadata = {
            'displayName': f'BattleMaps {args.version}',
    ...
    archive = directory / f'BattleMaps-{args.version}.zip'
    if not archive.is_file():
        sys.exit(f'Missing release archive: {archive}')
    if not metadata_path.is_file():
        sys.exit('Missing CurseForge metadata; run preflight first.')
    metadata = metadata_path.read_bytes()
    if json.loads(metadata)['displayName'] != f'BattleMaps {args.version}':
        sys.exit('Release metadata mismatch')

    boundary = 'BattleMaps' + uuid.uuid4().hex
    payload = (
    ...
    public_result = {
        'id': file_id,
        'url': f'https://www.curseforge.com/wow/addons/battle-maps/files/{file_id}',
    }
    ...
    if summary:
        with open(summary, 'a', encoding='utf-8') as file:
            file.write(
                f'CurseForge upload accepted: [{file_id}]({public_result["url"]}). '
                'Publication is subject to CurseForge processing/review.\n'
            )
